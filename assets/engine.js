...
function toggleHard(){
    /* settings.hardmode = (settings.hardmode) ? false : true;
    document.getElementById("s_hd").innerText = (settings.hardmode) ? "✓ Не выделять уже отмеченное" : "Не выделять уже отмеченное"; */
    settings.hardmode = s_hd.checked;
}

function toggleHelp(){
    /* settings.help = (settings.help) ? false : true;
    document.getElementById("s_hp").innerText = (settings.help) ? "✓ Показывать подсказки" : "Показывать подсказки"; */
...
function toggleSW(){
    if (settings.sw_tna) {
        ...
        settings.sw_enabled = s_sw.checked;//(settings.sw_enabled) ? false : true;
        //document.getElementById("s_sw").innerText = (settings.sw_enabled) ? "✓ Включить секундомер" : "Включить секундомер";
...
function loadData( area, spec ){
    ...
    xhr.onreadystatechange = function() {
        ...
        if (lfram.metadata.background != undefined) {
            ...
            console.log(lfram.metadata.background);
...
function parseData() {
    ...
    oneStep.query = ymaps.geoQuery(taskCollection); // Получает выборку всех объектов;
...
function drawGeoObjects( entry, ind ) {
    ...
    if (entry.geometry.type == "LineString") {
        ...
        //var newLine = new ymaps.Polyline( entry.geometry.coordinates, {strokeWidth: 2} );
        var newLine = new ymaps.GeoObject( {
                geometry: {
                    type: "LineString",
        ...
        backgroundCollection.add(newLine);
        if (entry.properties.backonly) return;
    ...
    /*if (entry.geometry.type == "MultiLineString") {
        //console.log(entry);
        if (settings.invert_coords) {
            for (var j in entry.geometry.coordinates) {
        ...
        for (var k in entry.geometry.coordinates) {
            var newLine = new ymaps.GeoObject( {
                geometry: {
                    type: "LineString",
                    coordinates: entry.geometry.coordinates[k], 
                },
                properties: { description: entry.properties.name },
            }, {strokeWidth: 2} 
                );
            backgroundCollection.add(newLine);
        }
        if (entry.properties.backonly==1) return;
        var half = Math.floor( entry.geometry.coordinates.length/2 );
        var quarter = Math.floor( entry.geometry.coordinates[half].length/2 );
        var newDot = new ymaps.GeoObject( {
            geometry: {
                type: "Point",
                coordinates: entry.geometry.coordinates[half][quarter] },
            id: entry.id,
                } );
        newDot.properties.balloonContentHeader = entry.properties.description;
        newDot.id = entry.id;
...
function oneStep() {
    ...
    oneStep.query.removeEvents('click', alertMistake);
    oneStep.query.addEvents('click', alertMistake);
    //console.log(oneStep.query);
    ...
    if (file && fileReader) {
        fileReader.addEventListener("loadend", function(e) {
            ...
            //try {
                ...
                //alert(settings.invert_coords);
                parseData();
            /* }
            catch(err) {
                alert("Во время обработки файла произошла ошибка! Точно ли вы пытаетесь загрузить GeoJSON?");
            } */
        });
